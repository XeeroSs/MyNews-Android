...
public class RecyclerViewAdapterMost extends RecyclerView.Adapter<RecyclerViewAdapterMost.MyViewHolder> {

    Context context;
    List<Articles.Result> articles = new ArrayList<>();
    private NotificationManager notifManager;

    // Constructor
    public RecyclerViewAdapterMost(Context c, List<Articles.Result> article) {
        context = c;
        articles = article;
    }

    public RecyclerViewAdapterMost() {
    }
    ...
    public Articles.Result getPosition(int position) {
    ...
    public void updateAnswers(Articles items) {
        articles.clear();
        articles.addAll(items.getResult());
        notifyDataSetChanged();
    }

    // Creation new views
    @Override
    public RecyclerViewAdapterMost.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        if (articles.get(position).getColor() != null) {
            holder.color.setBackgroundColor(Color.parseColor(articles.get(position).getColor()));
        }
