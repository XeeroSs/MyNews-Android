...
public class RecyclerViewAdapterTop extends RecyclerView.Adapter<RecyclerViewAdapterTop.MyViewHolder> {
    ...
    // Constructor
    public RecyclerViewAdapterTop(Context c, List<Articles.Result> article) {
        context = c;
        articles = article;
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        try {
            Picasso.with(context).load(articles.get(position).getMultimedias().get(0).getUrl()).into(holder.image);
        } catch (Exception e) {
            e.printStackTrace();
        }
